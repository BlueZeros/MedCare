...
import os 
import torch
from medcare.model.builder import load_molora_pretrained_model
...
def convert_to_automodel(model_path, model_base, load_8bit=False, load_4bit=False, use_logit_bias=False, device_map="auto", device="cuda", save_path=None):
    ...
    tokenizer, model, context_len, tokenizer_with_adapter = load_molora_pretrained_model(model_path, model_base, model_name, only_load=args.only_load)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only_load", choices=["attn", "share", "only_share"], required=True)
