...
from medcare.conversations import get_default_conv_template
import pdb
from medcare.model.utils import get_mixoflora_model, get_orthlora_model
import warnings
from transformers.models.qwen2.modeling_qwen2 import Qwen2DecoderLayer
from medcare.train.trainer import MedCareTrainer
from medcare.model import MoLoRAQwenForCausalLM
...
class ModelArguments:
    ...
    wrap_attn_orth: Optional[bool] = field(default=False)
    wrap_ffn_orth: Optional[bool] = field(default=True)
...
def train():
    ...
    if training_args.use_orthogonal:
        config.wrap_attn_orth = model_args.wrap_attn_orth
        config.wrap_ffn_orth = model_args.wrap_ffn_orth
    ...
    if training_args.lora_enable:
        ...
        if len(lora_config.target_modules) != 0 and "taia" not in model_args.model_name_or_path and "share" not in model_args.model_name_or_path:
    ...
    trainer = MedCareTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
